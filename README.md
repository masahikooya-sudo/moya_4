...
| CREDIT_CARD | クレジットカード番号 | 正規表現 + Luhnチェック(独自実装) |
...
- ファイルをアップロードしてマスキングし、ダウンロード
  ...
  - **TXT/PDFは、ラベルが単独行として現れ直後の行に値が続くフォーム的なレイアウト
    (履歴書等)を検出し、値部分を該当エンティティとして補完的に強制マスキングします**。
    「ふりがな」「氏名」等の短い値はNERだけでは検出漏れしやすいための補完機能で、
    列名ベースの判定と同じキーワード(`app/column_matcher.py`)を流用しています
    (実装: `app/file_processing.py` の `_label_forced_results`)。
...
既定では直近90日分を保持し、それより古いファイルは自動削除されます
...
- PDFは検出した文字列と同一のテキストをページ内で検索して黒塗りします。改行等で
  文字列が分断されている場合は、空白・改行区切りの断片ごとに個別検索するフォールバック
  処理で多くのケースを補っていますが、フォントの文字埋め込みが不完全なPDF(特定の文字が
  テキストとして抽出できない等)では、そもそも検出対象の文字列がテキスト層に存在しないため
  検出・マスキングできません。また黒塗り部分に重ねるラベルは日本語CJKフォント(`japan-s`)で
  描画しています。
- 履歴書等のフォーム的なPDF/テキストでは、ラベル(「氏名」「ふりがな」等)直後の値を
