...
- 直接入力したテキストのマスキング
- `.txt` / `.csv` ファイルをアップロードしてマスキングし、ダウンロード
  - CSVは行・列構造を保持したまま各セルをマスキングします
  - 文字コードは UTF-8 (BOM有無) / Shift_JIS(cp932) / EUC-JP を自動判定します
...
- CSVは現状セル単位で逐次解析しているため、行数の多いファイルは処理に時間がかかります。
